...
export async function POST(req: NextRequest) {
  try {
    const admin = await requireAdmin(); const input = schema.parse(await req.json()); const { reason, multiplier, ...values } = input; await connectMongo();
    const before = await ModelPriceModel.findOne({ model: input.model }).lean();
    // 未传 multiplier（如运营面板旧表单）时保留原值，避免把倍率重置为默认
    const set = { ...values } as Record<string, unknown>;
    if (multiplier !== undefined) set.multiplier = multiplier;
    const price = await ModelPriceModel.findOneAndUpdate({ model: input.model }, { $set: set }, { upsert: true, new: true, setDefaultsOnInsert: true });
